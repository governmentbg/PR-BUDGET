...
namespace CielaDocs.SjcWeb.Areas.ProsecutorAdmin.Controllers
{
    ...
    {
        private readonly ILogger<ProgramDataItemsController> _logger;
        ...
        public ProgramDataItemsController(ILogger<ProgramDataItemsController> logger, IConfiguration configuration, ISendGridMailer emailSender,
        ...
        public async Task<JsonResult> RecalculateGrid(int? functionalSubAreaId, int? nyear)
        {
            try
            {
                ...
                _ = await _sjcRepo.sp_RecalculateProgramDataAsync(functionalSubAreaId ?? 0, nyear + 3 ?? 0);
