...
EduDl.stores = {
    mymessagesstore:  new DevExpress.data.CustomStore({
        key: 'Id',
        load() {
            return sendRequest(EduDl.baseUrl + "MyMessagesDx/GetMyMessagesByEmplId", 'Get', { emplId: EduDl.EmplId });
        }
    })
    ,
    mymessagesOutStore: new DevExpress.data.CustomStore({
        key: 'Id',
        load() {
            return sendRequest(EduDl.baseUrl + "MyMessagesOutDx/GetMyMessagesOutByEmplId", 'Get', { emplId: EduDl.EmplId });
        }
    })
    ,
    myattorneyStore: new DevExpress.data.CustomStore({
        key: 'Id',
        load() {
            return sendRequest(EduDl.baseUrl + "MyAttorneyDx/GetMyAttorneyMessagesByEmplId", 'Get', { emplId: EduDl.EmplId });
        }
    })
};
    ...
    EduDl.processFilter = function() {
        ...
        $(".left-menu:not(#custom-filters) a").each(function () {
            ...
            if (value === '0') {
                ...
                EduDl.filterGrid(['DocTypeDtos.Id', '=', value], 'custom');
