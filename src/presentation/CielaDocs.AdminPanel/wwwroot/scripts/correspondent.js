...
var Correspondent = function () {
    ...
    var cardStore = EduDl.stores.corrStore,
        cardDataSource = {
            store: cardStore,
        };
                    ...
                    .dxDataGrid({
                        ...
                        dataSource: new DevExpress.data.DataSource({
                            store: new DevExpress.data.CustomStore({
                                key: 'Id',
                                load() {
                                    return sendRequest(EduDl.baseUrl + "CorrespondentDx/GetCardsByCorrId", 'Get', { corrId: options.key });
                                }
                            }),
                        }),
                        paging: {
    ...
    async function getExtraDataItem(row) {
        ...
        $('#card-info').empty();
        //$.ajax({
        //    url: EduDl.baseUrl + "DeloDx/GetMessageRegistered", data: { msgId: row.data.Id }, success: function (result) {
    ...
    function addNewCorr() {
        let corr_url = EduDl.baseUrl + "CorrespondentDx/AddCorr";
        corrNewContent.empty();
        corrNewContent.html('<p>Зареждам данни…</p>')
        $.ajax({
            type: 'Get',
            url: corr_url,
            data: { onrId: EduDl.OnrId },
    ...
    function editCorr(id) {
        let corr_url = EduDl.baseUrl + "CorrespondentDx/EditCorr";
        corrNewContent.empty();
        corrNewContent.html('<p>Зареждам данни…</p>')
        $.ajax({
            type: 'Get',
    ...
    function deleteCorr(id) {
        if (confirmDlg('Потвърдете изтриването на записа', function (ret) {
            if (ret) {
                let delurl = EduDl.baseUrl + "CorrespondentDx/Delete";
                $.ajax({
                    url: delurl, data: { onrId: EduDl.OnrId, id: id }, method: 'POST'
                })
                    .done(function (data) {
