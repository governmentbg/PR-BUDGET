...
{
    public class CurrentIndicatorDataController : Controller
    {
        private readonly ILogger<CurrentIndicatorDataController> _logger;
        ...
        public CurrentIndicatorDataController(ILogger<CurrentIndicatorDataController> logger, IConfiguration configuration, ISendGridMailer emailSender,
        ...
        public async Task<JsonResult> GetDataGrid(int? nyear)
        {
            ...
            {
                FilterData = HttpContext.Session.Get<FilterMainDataVm>("FilterMainDataSess") ?? new FilterMainDataVm();
                var data = await _sjcRepo.GetIndicatorData1YAsync(FilterData?.FunctionalSubAreaId ?? 0, nyear ?? 0);
        ...
        public async Task<JsonResult> GetCourtsByIndicatorDataId(int? nyear, int? indicatorDataId)
        {
            ...
            var data = await _sjcServiceV2.GetIndicatorDataCourt1YAsync(mIndicator?.FunctionalSubAreaId ?? 0, nyear ?? 0, indicatorData?.MainIndicatorId ?? 0);
            return Json(data.ToList());
        }
        ...
        public async Task<JsonResult> UpdateDataItem(int key, string values)
        {
            ...
            if (!string.IsNullOrWhiteSpace(name))
            {
                ...
                _ = await _sjcServiceV2.UpdateIndicatorData1YValueByIdAsync(key, name, n, FilterData?.Nyear);
