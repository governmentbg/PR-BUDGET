...
namespace CielaDocs.AdminPanel.Areas.Admin.Controllers
{
    ...
    public class CourtsController : Controller
    {
        ...
        public CourtsController(IMediator mediator, IMapper mapper, ILogRepository logRepo, ISjcBudgetRepository sjcRepo, IHttpContextAccessor httpContextAccessor)
        {
            _mediator = mediator;
            _mapper = mapper;
            _logRepo = logRepo;
        ...
        public async Task<IActionResult> Index()
        {
            ...
            string logmsg = $"Достъп до органи на съдебната власт от {User?.Identity?.Name}";
            await _logRepo.AddToAppUserLogAsync(new Domain.Entities.AppUserLog { AppUserId = user?.Id ?? 0, MsgId = 0, Msg = logmsg, IP = ip });
            return View();
        }

        [HttpGet]
        public PartialViewResult AddCourtPartial()
        {

            return PartialView("AddCourtPartialView", new CourtDto { Id = 0, CourtGuid = Guid.NewGuid().ToString("N") });


        }
        [HttpGet]
        public async Task<PartialViewResult> EditCourtPartial(int? id)
        {
            if ((id == null) || (id < 0))
            {
                return PartialView("_ErrorPartialView", "Невалиден указател към съдебен орган!");
            }
            var court = await _mediator.Send(new GetCourtByIdQuery { Id = id ?? 0 });
            court.KontoCode=court?.KontoCode?.Trim();
            return PartialView("AddCourtPartialView", court);
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Save(CourtDto record)
        {

            if (ModelState.IsValid)
            {

                var onr = await _mediator.Send(new GetCourtByGuidQuery { CourtGuid = record.CourtGuid });
                ...
                if (onr == null)
                {
                    ...
                    try
                    {
                        ...
                        CreateCourtCommand command = new CreateCourtCommand
                        {
                            ...
                            KontoCode=record?.KontoCode ?? string.Empty,
                        };
        ...
        List<CustErrors> GetCustErrors(CourtDto data)
        {
            ...
            if (data?.CourtTypeId < -1)
            {
                ret.Add(new CustErrors { Name = "Изберете вид на органа на съдебна власт!" });
            }
            return ret;
        }
