...
{
    public class MainPeriodItmController : Controller
    {
        ...
        public async Task<JsonResult> GetMainPeriodGrid()
        {
            try
            {
                FilterData = HttpContext.Session.Get<FilterMainDataVm>("FilterMainDataSess") ?? new FilterMainDataVm();
                var data = await _sjcRepo.GetMainPeriodItemsGridByFilterAsync(FilterData?.CourtId ?? 0, FilterData?.Nmonth ?? 0, FilterData?.Nyear ?? 0);
        ...
        [HttpGet]
        public async Task<PartialViewResult> GetMainPeriodItemsPartial(int? id)
        {
            if ((id == null) || (id < 0))
            {
        ...
        public async Task<JsonResult> UpdateMainPeriodItem(int key, string values)
        {
            var nv = new Nvalues();
            JsonConvert.PopulateObject(values, nv);
            _ = await _sjcRepo.UpdateMainPeriodItemValueByIdAsync(key, nv.Nvalue);
                ...
                foreach (DataTable thisTable in data.Tables)
                {
                    ...
                    foreach (DataRow row in thisTable.Rows)
                    {
                        ...
                        foreach (DataColumn column in thisTable.Columns)
                        {
                            ...
                            sCode = row["Код"].ToString();
                            sValue = row["Стойност"]?.ToString();



                            if (!string.IsNullOrWhiteSpace(sValue))
                            {
                                decimal.TryParse(sValue, out decimal nVal);
                                var rec = dbdata.Where(x => x.Code == sCode).FirstOrDefault();
                                if (rec != null)
                                {
                                    if (rec?.EnteredOn == null)
                                    {
                                        _ = await _sjcRepo.UpdateMainPeriodItemValueByIdAsync(rec?.Id, nVal);
                                        nCnt++;
                                    }
