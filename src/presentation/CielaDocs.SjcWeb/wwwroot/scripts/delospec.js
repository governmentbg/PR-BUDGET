...
var SignalSpec = function () {
    ...
    var cardStore = Ciela.stores.delospecstore,
        cardDataSource = {
            store: cardStore,
        };
    ...
    $('<div>')
        ...
        .text(`${currentCardRowData.DocNum} ,№ ${} Свързани преписки:`)
        .appendTo(container);
        ...
        .dxDataGrid({
            ...
            dataSource: new DevExpress.data.DataSource({
                store: new DevExpress.data.CustomStore({
                    key: 'Id',
                    load() {
                        return sendRequest(Ciela.baseUrl + "DeloSpecDx/GetCardsByMasterId", 'Get', { onrId: Ciela.OnrId, cardId: options.key });
                    }
                }),
            }),
            paging: {
    ...
    async function getExtraDataItem(row) {
        ...
        $('#card-info').empty();
        $.ajax({
            url: Ciela.baseUrl + "DeloSpecDx/GetCardDetails", data: { cardId: row.data.Id }, success: function (result) {
    ...
    function addCardAction(actionId) {
        ...
        switch (actionId) {
            case "1": 
                ...
                break;
            case "2": {
                let checkCardUrl = Ciela.baseUrl + "CardAction/CheckCardEnabledForReply";
                ...
                $.ajax({
                   ...
                   break;
            case "3": {
                let urlcompl = Ciela.baseUrl + "CardAction/GetCardForCompletion";
                ...
                break;
            case "4": {
                let urlcompl = Ciela.baseUrl + "CardAction/GetCardForArchiving";
                cardActionContent.empty();
                cardActionContent.html('<p>Зареждам данни…</p>')
                $.ajax({
                    type: 'POST',
    ...
    function addCardReplyNav(cardMasterId) {
        let cardreplyurl = Ciela.baseUrl + "DeloSpecDx/GetCardNavMasterPartial";
        commonContent.empty();
    ...
    function addCardLinkedNav(cardMasterId) {
        let cardlinkurl = Ciela.baseUrl + "DeloSpecDx/GetCardNavMasterPartial";
        commonContent.empty();
        commonContent.html('<p>Зареждам данни…</p>')
        $.ajax({
            type: 'Get',
    ...
    function addCardMovement(movementId) {
        ...
        switch (movementId) {
            case "0": {
                ...
                let empldirectUrl = Ciela.baseUrl + "EmplDirect/AddMessagePartialView";
                commonContent.empty();
                commonContent.html('<p>Зареждам данни…</p>')
                $.ajax({
                    type: 'Get',
            ...
            case "1": {
                ...
                let empldirectUrl = Ciela.baseUrl + "EmplDirect/DocToEmplDxPartialView";
                    commonContent.empty();
                    commonContent.html('<p>Зареждам данни…</p>')
                    $.ajax({
                        type: 'Get',
                ...
                break;
            case "2":
                let cardmovesUrl = Ciela.baseUrl + "EmplDirect/LoadCardMovementGrid/?cardId=" + selectedCardId;
                window.open(cardmovesUrl, '_blank');
                break;
        }
    }
    function addCardNav(docTypeId) {
        let cardurl = Ciela.baseUrl + "DeloSpecDx/GetCardNavPartial";
        cardContent.empty();
        cardContent.html('<p>Зареждам данни…</p>')
        $.ajax({
            type: 'Get',
            url: cardurl,
            data: { onrId: Ciela.OnrId, docTypeId: docTypeId },
            success: function (data) {
    ...
    function editCardNav(id) {
        let cardurl = Ciela.baseUrl + "DeloSpecDx/GetCardNavEditPartial";
         commonContent.empty();
        commonContent.html('<p>Зареждам данни…</p>')
        $.ajax({
            type: 'Get',
            ...
            onItemClick(data) {
                ...
                if (item.id) {
                    switch (item.id) {
                        ...
                        case '7': addCardFilter(Ciela.OnrId);
                            break;
                    }
                }
            },
