...
namespace CielaDocs.SjcWeb.Controllers
{
    ...
    public class DocArchiveController : Controller
    {
        ...
        public IActionResult OpenFile(string name)
        {
            if (!string.IsNullOrEmpty(name))
            {
                ...
                if (items?.Length == 1)
                {
                    ...
                    if (fileExtension.EndsWith("docx") || fileExtension.EndsWith("rtf") || fileExtension.EndsWith("txt"))
                    {

                        return RedirectToAction("Index", "WordViewer", new { id = "", filePath = filePath });
                    }
                    else if (fileExtension.EndsWith("xlsx") || fileExtension.EndsWith("xlsm"))
                    {
                        return RedirectToAction("Index", "ExcelViewer", new { id = "", filePath = filePath });
                    }
                    else if (fileExtension.EndsWith("pdf")) {
                        var pdffilepath = filePath.Replace("\\", "/");
                        var subfile=string.Empty;
                        if (pdffilepath.Contains("localdocarchive")) {
                            int start;
                            start = pdffilepath.IndexOf("localdocarchive", 0);
                            subfile = pdffilepath.Substring(start, pdffilepath.Length-start);
                        }
                        var request = _httpContextAccessor?.HttpContext?.Request;
                        var domain = $"{request?.Scheme}://{request?.Host}";
                        var viewerurl = $"{domain}/PdfViewer/web/viewer.html?file=../../{subfile}";
                        byte[] bytes = Encoding.ASCII.GetBytes(viewerurl);
                        string encodedUrl = Convert.ToBase64String(bytes);
                        return RedirectToAction("Index", "PdfViewer", new { id = encodedUrl });
                    }
                }
                else
                {
                    return RedirectToAction("NotSupportedFile", "Home");
                }
            }
            return RedirectToAction("NotSupportedFile", "Home");
        }
