        ...
        public async Task<JsonResult> LoadExcelFileToTable(SpreadsheetClientState spreadsheetState, int? importType, string? fileName)
        {
            ...
            else if (importType == 1)
            {
                ...
                try
                {
                    ...
                    string sFileNameOnly = System.IO.Path.GetFileNameWithoutExtension(id);
                    var excelHeaderData = GetExcelFileInstitutionHeaderByName(fileName);

                    int nYear = (int)excelHeaderData.Item2;
                    int.TryParse(excelHeaderData.Item1, out int nInstitutionTypeId);
                    var inst = await _sjcRepo.GetInstitutionTypeByIdAsync(nInstitutionTypeId);
                    if (inst == null)
                    {
                        return Json(new { msg = $"Неоткрит код {excelHeaderData.Item1} на отчетна единица", success = false });
                    }
                    ...
                    if (instInPrograms.Any())
                    {
                        foreach (var item in instInPrograms)
                        {
                            ...
                            foreach (var yearItem in yearsLst)
                            {
                                ...
                                //always do this nullify values
                                _ = await _sjcRepo.FirstInitProgramDataDraftBudgetProsecutorAsync(item?.InstitutionTypeId, item?.FunctionalSubAreaId ?? 0, yearItem);
                                ...
                                if (programDefCodes.Any())
                                {
                                    ...
                                    foreach (var prowDef in programDefCodes)
                                    {
                                        ...
                                        // s += $"CourtId={item?.CourtId},FunctionalSubAreaId={prowDef?.FunctionalSubAreaId ?? 0},rowNum={prowDef?.RowNum},nYear={yearItem}, nval={nval}, progCode={progCode}" + Environment.NewLine;
                                        _ = await _sjcRepo.ProgramDataDraftBudgetProsecutorAsync(item?.InstitutionTypeId, prowDef?.FunctionalSubAreaId ?? 0, prowDef?.RowNum, yearItem, nval);
                                        nCnt++;
                                    }
                                }
                                _ = await _sjcRepo.sp_RecalculateProgramDataProsecutorAsync(item?.FunctionalSubAreaId, yearItem, item?.InstitutionTypeId);
                            }
                            //-------------
                        }
                    }
