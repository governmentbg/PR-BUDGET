...
var Messages = function () {
    ...
    var msgStore = EduDl.stores.mymessagesstore,
        msgDataSource = {
            store: msgStore,
        };
        ...
        columns: [

            {
                ...
                cellTemplate: function (element, info) {
                    if (info.text == 'Не') {
                        element.append("<div><img alt='.' src='./images/mailunreaded.png'></img></div>");
                    }
                    else {
                        element.append("<div><img alt='.' src='./images/mailreaded.png'></img></div>");
                    }
                }
            },
            {
    ...
    function addCardReplyNav(cardMasterId) {
        let cardreplyurl = EduDl.baseUrl + "DeloDx/GetCardNavMasterPartial";
        commonContent.empty();
    ...
    function addCardLinkedNav(cardMasterId) {
        let cardlinkurl = EduDl.baseUrl + "DeloDx/GetCardNavMasterPartial";
        commonContent.empty();
        commonContent.html('<p>Зареждам данни…</p>')
        $.ajax({
            type: 'Get',
    ...
    function addCardNav(docTypeId) {
        let cardurl = EduDl.baseUrl + "DeloDx/GetCardNavPartial";
        cardContent.empty();
    ...
    function editCardNav(id) {
        let cardurl = EduDl.baseUrl + "DeloDx/GetCardNavEditPartial";
        cardContent.empty();
        cardContent.html('<p>Зареждам данни…</p>')
        $.ajax({
            type: 'Get',
                        ...
                        case '6_1':
                            ...
                            DevExpress.excelExporter.exportDataGrid({
                                ...
                                workbook.xlsx.writeBuffer().then(function (buffer) {
                                    saveAs(new Blob([buffer], { type: "application/octet-stream" }), "Messages.xlsx");
                                });
                            });
                            break;
