    ...
    public class NomsController : Controller
    {
        ...
        public async Task<JsonResult> GetInstitutionTypes()
        {
            try
            {
                ...
                return Json(data.Where(x=>x.Id==2).ToList());
            }
            catch (Exception ex)
            {
        ...
        public async Task<JsonResult> GetCourtTypes(string term = "")
        {
            ...
            var returner = await _sjcService.QueryRawList<IdNames>($@"SELECT  a.Id,a.Name FROM CourtType a
                left join InstitutionType i on a.InstitutionTypeId=i.id
                where i.id=2 and a.Name like '{term}%'");
            return Json(returner.ToArray());
        ...
        public async Task<JsonResult> GetCourtTypesByProgramId(int? programId, string term = "")
        {
            string sql = $@"select distinct(a.Id),a.Name
                  ...
                  left join InstitutionType i on a.InstitutionTypeId=i.id
                  where i.id=2 and p.FunctionalSubAreaId={programId ?? 0} and a.Name like '{term}%'";

            var returner = await _sjcService.QueryRawList<IdNames>(sql);
            return Json(returner.ToArray());
        ...
        public async Task<JsonResult> GetFunctionalAreas(string term = "")
        {
            ...
            var returner = await _sjcService.QueryRawList<IdNames>($"Select Id,Name from FunctionalArea where id in(3,4,6)");
            return Json(returner.ToArray());
        ...
        public async Task<JsonResult> GetAllFunctionalSubAreas()
        {
            ...
            return Json(returner.Where(x=>x.Id==6||x.Id==7||x.Id==9).ToArray());
        }
