...
{
    public class IndicatorDataCourtItemsController : Controller
    {
        private readonly ILogger<IndicatorDataCourtItemsController> _logger;
        ...
        public IndicatorDataCourtItemsController(ILogger<IndicatorDataCourtItemsController> logger, IConfiguration configuration, ISendGridMailer emailSender,
        ...
        public async Task<JsonResult> GetDataGrid(int? programNum, int? courtId, int? nyear)
        {
            try
            {
                ...
                var data = await _sjcServiceV2.GetIndicatorDataCourt3YByCourtIdAsync(programNum, nyear ?? 0, courtId ?? 0);
